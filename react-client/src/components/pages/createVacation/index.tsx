...
import { createVacationApi, } from './service';
...
export default function CreateVacationForm() {
    ...
    function isCountryValid() {
     ...
     function isImageValid() {
        const result = ImageScheme.safeParse(imageUrl);
        if (result.success) {
    ...
    function isSubmitDisabled(): boolean {
        ...
        if (countryError.isError || priceError.isError || cityError.isError || descriptionError.isError||imageError.isError) {
    ...
    const handleSubmit = async () => {
        try {
            setIsLoading(true);
           const result = await createVacationApi({
